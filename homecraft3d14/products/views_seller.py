    ...
    if not is_owner_user(request.user):
        ...
        if not stripe_ready:
            ...
            if active_free_count >= cap:
                if not bool(getattr(request.user, "email_verified", False)):
                    messages.warning(
                        request,
                        "Please verify your email to publish more free file listings. "
                        "Email verification is required before Stripe onboarding.",
                    )
                    return redirect("accounts:verify_email_status")
