...
class SiteConfigForm(forms.ModelForm):
        ...
        widget=forms.Textarea(
            attrs={
                ...
                "placeholder": (
                    ...
                    "  {\"label\": \"Bambu Lab Nozzle\", \"url\": \"https://...\"}\n"
                    "]"
                ),
            }
        ),
        help_text="JSON array of affiliate links. Each item requires label+url; note optional.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if inst and inst.pk:
            ...
            try:
                raw_links = getattr(inst, "affiliate_links", None) or []
                self.fields["affiliate_links"].initial = json.dumps(raw_links, indent=2)
            except Exception:
                self.fields["affiliate_links"].initial = ""
    ...
    def clean_affiliate_links(self) -> list[dict]:
        raw = (self.cleaned_data.get("affiliate_links") or "").strip()
        if not raw:
            return []
        try:
            parsed = json.loads(raw)
        except Exception as exc:
            raise forms.ValidationError("Affiliate links must be valid JSON.") from exc
        if not isinstance(parsed, list):
            raise forms.ValidationError("Affiliate links JSON must be an array.")
        cleaned: list[dict] = []
        for item in parsed:
            if not isinstance(item, dict):
                raise forms.ValidationError("Each affiliate link must be an object.")
            label = str(item.get("label", "")).strip()
            url = str(item.get("url", "")).strip()
            note = str(item.get("note", "")).strip() if item.get("note") is not None else ""
            if not label or not url:
                raise forms.ValidationError("Each affiliate link requires 'label' and 'url'.")
            cleaned_item = {"label": label, "url": url}
            if note:
                cleaned_item["note"] = note
    ...
    def save(self, commit: bool = True) -> SiteConfig:
        ...
        obj.affiliate_links = self.cleaned_data.get("affiliate_links") or []
