...
from .models import LineItem, Order, OrderEvent, SellerFulfillmentTask
...
def ensure_fulfillment_tasks_for_paid_order(*, order: Order) -> None:
    """Create (or reopen) seller fulfillment tasks for a paid order.

    Only applies to physical items (requires_shipping=True) and excludes tip lines.
    """
    sellers = (
        order.items.filter(requires_shipping=True, is_tip=False)
    ...
    for seller_id in sellers:
        if not seller_id:
            continue
        task, created = SellerFulfillmentTask.objects.get_or_create(
            seller_id=seller_id,
            order_id=order.id,
            defaults={"is_completed": False},
        )
        if not created and task.is_completed:
            # If an admin reopens fulfillment (rare), allow task to become active again.
...
def refresh_fulfillment_task_for_seller(*, order: Order, seller_id) -> None:
    """Mark the fulfillment task completed if seller has no pending shippable items."""
    try:
        task = SellerFulfillmentTask.objects.filter(order_id=order.id, seller_id=seller_id).first()
    except Exception:
        task = None
    if not task:
        return

    pending_exists = order.items.filter(
        seller_id=seller_id,
