...
def _send_seller_new_order_email(order: "Order") -> None:
    """Notify sellers when they receive a paid order.

    - Sends BOTH email + in-app notification (via notify_email_and_in_app).
    - Tip lines are excluded.
    - For physical items, includes shipping address and fulfillment guidance.
    - For digital-only sales, still notifies the seller (no shipping section).
    """
    sellers = {}
    for item in order.items.filter(is_tip=False).select_related("seller", "product"):
        seller = item.seller
        if not seller or not getattr(seller, "email", ""):
            continue
        sellers.setdefault(seller.id, {"seller": seller, "items": [], "has_physical": False})
        sellers[seller.id]["items"].append(item)
        if item.requires_shipping:
            sellers[seller.id]["has_physical"] = True
    ...
    base = _site_base_url()
    logo_url = _absolute_static_url("images/homecraft3d_icon.svg")
    detail_url = reverse("orders:seller_order_detail", kwargs={"order_id": order.pk})
    seller_order_link = f"{base}{detail_url}"

    for info in sellers.values():
        seller = info["seller"]
        items = info["items"]
        has_physical = bool(info["has_physical"])
        ...
        if has_physical:
            ...
            subject = f"New sale - #{order.pk}"
            lines.append(f"New sale! Order #{order.pk}")

        lines.append("")
        lines.append("View order details:")
        lines.append(seller_order_link)
        lines.append("")
        lines.append("Items:")
        for item in items:
            kind = "Digital" if item.is_digital else "Physical"
            lines.append(f"- {item.quantity}x {item.product.title} ({kind}) (${item.line_total_cents / 100:.2f})")
        lines.append("")

        shipping_lines = ""
        if has_physical and order.requires_shipping:
            ...
            lines.append("No shipping is required for this sale.")
        ...
        context = {
            ...
            "logo_url": logo_url,
            "order_id": order.pk,
            "fulfillment_link": seller_order_link,
        ...
        notify_email_and_in_app(
            ...
            payload={"order_id": str(order.pk), "type": "seller_new_order", "has_physical": has_physical},
        )
...
class SellerFulfillmentTask(models.Model):
    """Persistent seller 'must-do' fulfillment task per paid order.

    Created when an order is PAID and contains at least one physical item (requires_shipping=True)
    for the seller. Stays open until all of that seller's shippable items in the order are no longer
    pending fulfillment.
    """
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["seller", "order"], name="uniq_fulfillment_task_per_seller_order"),
        ]
        indexes = [
            models.Index(fields=["seller", "is_completed", "created_at"]),
        ]
        ordering = ("-created_at",)
