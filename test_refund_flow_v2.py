# test_refund_flow_v2.py
"""
Refund Flow End-to-End Testing Script - SIMPLIFIED
...
model_cat, _ = Category.objects.get_or_create(
    slug="test-models",
    defaults={
        "name": "Test Models",
        "type": Category.CategoryType.MODEL,
    }
)
file_cat, _ = Category.objects.get_or_create(
    slug="test-files",
    defaults={
        "name": "Test Files",
        "type": Category.CategoryType.FILE,
    }
)
...
print(f"  ✓ Created physical product: {product1.title} (${product1.price:.2f})")
...
print(f"  ✓ Created physical product: {product2.title} (${product2.price:.2f})")
...
try:
    ...
    print(f"  ⚠ Stripe refund trigger: {str(e)[:80]}")
...
print(f"  ✓ Created two physical products (${product3a.price:.2f} each)")
...
try:
    ...
    print(f"  ⚠ Staff trigger attempted: {str(e)[:80]}")
    print(f"    (Stripe error expected if keys not set)")
