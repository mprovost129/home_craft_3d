...
## 2026-02-09 — Change Pack: Launch Ops Observability (Webhooks + Refund Attempts)
- Added `orders.StripeWebhookDelivery`:
  - records Stripe webhook receipt + processing outcome (`received/processed/error`)
  - stores `request_id`, `processed_at`, and `error_message` for debugging
  - webhooks now return **500** on internal processing exceptions so Stripe will retry.
- Added `refunds.RefundAttempt`:
  - logs each refund trigger attempt (`success/error`) with actor + error details
  - powers Admin Ops visibility into refund failures without digging through logs.
- Added **Admin Ops** dashboard page:
  - `/dashboards/admin/ops/` showing webhook status counts, recent webhook errors, refund attempts, and recent Order warning events.
