...
### Launch hardening (ops + reconciliation)
- [x] Admin Ops dashboard: webhook outcomes + refund attempt failures + warning events.
- [ ] Admin reconciliation screens: per-order ledger vs Stripe transfers (downloadable CSV).
- [ ] Seller payout detail view (history, last transfer ids, net vs fees).
- [ ] Refund/chargeback tooling: staff queue + seller guidance copy.
